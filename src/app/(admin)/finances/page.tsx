...
export default async function FinancesPage() {
  ...
  const [thisMonth, lastMonth, topServices] = await Promise.all([
    prisma.appointment.findMany({
      where: { date: { gte: startOfMonth, lte: endOfMonth }, status: "COMPLETED" },
  ...
  const thisMonthRevenue = thisMonth.reduce((sum, a) => sum + Number(a.totalPrice), 0);
  const lastMonthRevenue = lastMonth.reduce((sum, a) => sum + Number(a.totalPrice), 0);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-1">Resumo financeiro do gabinete</p>
      </div>
        ...
        <div className="card p-6">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-2xl font-bold mt-1">{formatCurrency(thisMonthRevenue)}</p>
              <p className="text-xs text-gray-400 mt-1">{thisMonth.length} serviços concluídos</p>
            </div>
        ...
        <div className="card p-6">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-2xl font-bold mt-1">{formatCurrency(lastMonthRevenue)}</p>
              <p className="text-xs text-gray-400 mt-1">{lastMonth.length} serviços concluídos</p>
            </div>
      ...
      <div className="card p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="font-heading font-semibold">Serviços Mais Populares</h2>
        </div>
              ...
              <div key={ts.serviceId} className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-sm text-gray-400">{ts._count.serviceId}x</span>
                  </div>
                  <div className="h-2 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                    <div className="h-full rounded-full gradient-bg transition-all duration-500" style={{ width: width + "%" }} />
                  </div>
                </div>
              </div>
            );
